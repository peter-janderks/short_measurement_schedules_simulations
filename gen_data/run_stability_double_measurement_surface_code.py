...
def get_normal_data(diameters, rounds, error_probabilities, measurement_noise_factor):
    samples_normal = sinter.collect(
        ...
        decoders=["pymatching"],
        save_resume_filepath=f"./data/18_5_data/stability_double_measurement_surface_code.csv",
        print_progress=True,
    )
...
def generate_SD6_tasks(diameters, rounds, error_probabilities, measurement_noise_factor):
    for diameter in diameters:
        ...
        for p in error_probabilities:
            stim_circuit = StabilityDoubleMeasurementSurfaceCode(
                diameter, diameter, p, 0,  p, p, p*measurement_noise_factor, p).builder.circ
            circuit_path = "stim_circuits/stability_double_measurement_surface_code_SD6_d" + \
            ...
            yield sinter.Task(
                ...
                json_metadata={"p": p, "diameter": diameter, "code": "double_measurement_surface_code",
                               "noise_model": "SD6_" + str(measurement_noise_factor), "rounds": diameter, "measurement_noise_factor": measurement_noise_factor}
            )
...
def generate_SI1000_tasks(diameters, rounds, error_probabilities):
    for diameter in diameters:
        for p in error_probabilities:
            ...
            circuit_path = "stim_circuits/stability_double_measurement_surface_code_SI000_d" + \
                str(diameter) + "_p" + str(p) + \
                "_rounds" + str(diameter)+".stim"
            stim_circuit.to_file(circuit_path)
            yield sinter.Task(
                # get it for the stim example
                circuit=StabilityDoubleMeasurementSurfaceCode(
                    diameter, diameter, p/10, 2*p, p/10, p, 5*p, 2*p).builder.circ,
                json_metadata={"p": p, "diameter": diameter, "code": "double_measurement_surface_code",
