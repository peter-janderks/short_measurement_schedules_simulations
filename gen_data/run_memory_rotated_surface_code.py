    ...
    for d in distances:
        for p in error_probabilities:
            yield sinter.Task(
                ...
                circuit=RotatedSurfaceCode(d, p, 0,  p, p, measurement_noise_factor * p, p, rounds).builder.circ,
...
def generate_SI1000_tasks(distances, error_probabilities, rounds):
    for d in distances:
        for p in error_probabilities:
            yield sinter.Task(
                # get it for the stim example
                circuit=RotatedSurfaceCode(d, p/10, 2*p, p/10, p, 5*p, 2*p, rounds).builder.circ,
                json_metadata={"p": p, "d": d, "code": "rotated_surface_code",
...
def main():
    ...
    for distances, error_probabilities in data_to_collect.items():
        for measurement_noise_factor in measurement_noise_factors:
            get_normal_data(list(distances), list(error_probabilities), measurement_noise_factor, rounds=1)
