...
class StabilityDoubleMeasurementSurfaceCode(object):
    def __init__(self, diameter, rounds, p_idling, p_resonator_idling, p_1, p_2, p_m, p_prep):
        ...
        (self.data_qubits,
         ...
         self.builder.X_top_boundary_flags,
         self.builder.X_right_boundary_ancilla,
         self.builder.X_right_boundary_flags,
         self.builder.X_bottom_boundary_ancilla,
         self.builder.X_bottom_boundary_flags) = self.create_t0()
        ...
        self.builder.ancilla_qubits_ind_to_coord.update(
            {ind: coord for coord, ind in self.builder.X_top_boundary_flags.items()})
        self.builder.ancilla_qubits_ind_to_coord.update(
            {ind: coord for coord, ind in self.builder.X_bottom_boundary_ancilla.items()})
        self.builder.ancilla_qubits_ind_to_coord.update(
            {ind: coord for coord, ind in self.builder.X_bottom_boundary_flags.items()})
        ...
        all_qubits = (
            ...
            | set(self.builder.X_top_boundary_flags.values())
            | set(self.builder.X_bottom_boundary_ancilla.values())
            | set(self.builder.X_bottom_boundary_flags.values())
        )

        self.ancilla_qubits_without_flags_dict = (self.builder.X_face_ancilla | self.builder.Z_face_ancilla | self.builder.X_left_boundary_ancilla | self.builder.X_right_boundary_ancilla | self.builder.X_top_boundary_ancilla |
                                                  self.builder.X_bottom_boundary_ancilla)
        self.flag_qubit_dict = (self.builder.X_face_flags | self.builder.Z_face_flags | self.builder.X_right_boundary_flags | self.builder.X_left_boundary_flags |
                                self.builder.X_top_boundary_flags | self.builder.X_bottom_boundary_flags)
    ...
    def build_six_layers(self, all_qubits):
        self.builder.build_layer(self.l0_l6, all_qubits)
    ...
    def create_t0(self):
        ...
        (X_left_boundary_ancilla, X_left_boundary_flags, X_right_boundary_ancilla, X_right_boundary_flags,
         X_top_boundary_ancilla, X_top_boundary_flags, X_bottom_boundary_ancilla, X_bottom_boundary_flags, index) = self.create_X_boundary(index)
        ...
        initialized_qubits.extend(list(X_top_boundary_flags.values()))
        initialized_qubits.extend(list(X_bottom_boundary_ancilla.values()))
        initialized_qubits.extend(list(X_bottom_boundary_flags.values()))
        ...
        return (
            ...
            X_top_boundary_flags,
            X_bottom_boundary_ancilla,
            X_bottom_boundary_flags,
        )
    ...
    def z_face_detector(self, q_indexes_z_faces, coords_list, z_cor):
        for cor_index, q_index in enumerate(q_indexes_z_faces):
            coords = [coords_list[cor_index][0]-0.5, coords_list[cor_index][1]+0.5]
            self.builder.circ.append(
                "DETECTOR",
                [
                    stim.target_rec(self.builder.measurement_order[q_index]),
    ...
    def measure_data_qubits(self, z_cor, all_qubits):
        ...
        self.z_face_detector(list(self.builder.Z_face_flags.values()),
                             list(self.builder.Z_face_flags.keys()), z_cor)
    ...
    def l0_l6(self):
        ...
        for qubit in self.builder.X_top_boundary_flags.values():
            self.builder.H(qubit)

        for qubit in self.builder.X_bottom_boundary_ancilla.values():
            self.builder.H(qubit)

        for qubit in self.builder.X_bottom_boundary_flags.values():
            self.builder.H(qubit)
    ...
    def l5(self):
        ...
        for coords, q_index in self.builder.X_top_boundary_ancilla.items():
            self.builder.cnot(self.builder.X_top_boundary_flags[coords[0]+0.5, coords[1]-0.5], q_index)
        ...
        for coords, q_index in self.builder.X_bottom_boundary_ancilla.items():
            self.builder.cnot(self.builder.X_bottom_boundary_flags[coords[0]+0.5, coords[1]-0.5], q_index)
    ...
    def first_time_measuring_ancillas(self, all_qubits):
        ...
        qubits_to_measure.extend(list(self.builder.X_top_boundary_flags.values()))
        qubits_to_measure.extend(list(self.builder.X_bottom_boundary_ancilla.values()))
        qubits_to_measure.extend(list(self.builder.X_bottom_boundary_flags.values()))
    ...
    def create_X_boundary(self, index):
        ...
        for i in range(0, self.diameter - 1, 2):
            ...
            X_top_boundary_flags[starting_x + i +
                                 0.5, starting_y+i-0.5] = index + 1
            index += 2
        ...
        for i in range(0, self.diameter - 1, 2):
            ...
            X_bottom_boundary_flags[starting_x + i +
                                    0.5, starting_y+i-0.5] = index + 1
            index += 2
        ...
        self.builder.circ.append("R", X_top_boundary_flags.values())
        self.builder.circ.append("R", X_right_boundary_flags.values())
        self.builder.circ.append("R", X_bottom_boundary_flags.values())
        return (X_left_boundary_ancilla_qubits, X_left_boundary_flags, X_top_boundary_ancilla_qubits,
                X_top_boundary_flags, X_right_boundary_ancilla_qubits, X_right_boundary_flags, X_bottom_boundary_ancilla_qubits, X_bottom_boundary_flags, index)
    # oh I controlled s this!
